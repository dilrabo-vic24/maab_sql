...
from (SELECT
    ...
    -- YearInterval,
    Case 
        When YearInterval < 2 Or HireYear < 1975 Then Null
        When YearInterval >=2 And NextYear is not null Then 
            CONCAT(HireYear+1, '-',NextYear-1)
        When NextYear is null Then CONCAT(HireYear+1, '-',YEAR(GETDATE()))
    end as Years
from (SELECT
    YEAR(HIRE_DATE) as HireYear,
    Lead(YEAR(HIRE_DATE)) Over(Order By HIRE_DATE)as NextYear,
    (Lead(YEAR(HIRE_DATE)) Over(Order By HIRE_DATE) - YEAR(HIRE_DATE)) as YearInterval
from EMPLOYEES_N) as MyTable1) as MyTable2
